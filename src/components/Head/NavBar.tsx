    ...
    <ul className='flex flex-col w-full items-center lg:items-start'>
      {navigation.map(nav => {
        ...
        return (
          <li
            key={nav.label}
            className={`group h-10 text-xl flex items-center justify-center w-full px-2 hover:bg-white hover:text-secondary text-white hover:font-[600] ${isActive ? 'bg-secondary font-[600]' : ''}`}
          >
            <Link
              href={nav.target}
            ...
            >
              ...
              <span className='opacity-0 max-w-0 overflow-hidden invisible absolute left-0 lg:group-hover:opacity-100 lg:group-hover:max-w-[150px] lg:group-hover:visible lg:relative lg:opacity-100 lg:max-w-[150px] lg:visible'>
                {nav.label}
              </span>
            </Link>
          </li>
